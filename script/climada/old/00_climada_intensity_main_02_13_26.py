...
def read_single_storm_from_nc(nc_path, storm_index: int) -> xr.Dataset:
    """
    Slice one storm (track) from a multi-storm NetCDF file
    and subset to only valid (non-padded) time steps.

    Returns raw per-track Dataset (still needs normalization).
    """
    ds = xr.open_dataset(nc_path)

    if storm_index >= ds.sizes["n_trk"]:
        ds.close()
        raise IndexError(f"Storm index {storm_index} out of range")

    # Slice one track lazily
    ds_track = ds.isel(n_trk=storm_index)

    # Identify valid time steps using core variables
    core_vars = [
        "lon",
        "lat",
        "max_sustained_wind",
    ...
    # True where at least one variable is non-NaN
    valid_mask = np.any(
        [~np.isnan(ds_track[var].values) for var in core_vars],
        axis=0,
    )
    ...
    # Find contiguous valid time window
    valid_indices = np.where(valid_mask)[0]
    t0 = valid_indices[0]
    t1 = valid_indices[-1] + 1  # slice end is exclusive

    # Subset to valid time window and load only that slice
    ds_track = ds_track.isel(time=slice(t0, t1)).load()
...
def get_ordered_storm_indices_nc(nc_path: Path) -> list[int]:
    """
    Return storm indices from a multi-storm NC file, ordered chronologically
    ...
    for storm_index in range(n_storms):
        ...
        sid = storm_index  # we can use index as stable tiebreaker

        storm_entries.append((storm_index, start_date, duration, sid))

    # --- Sort by start_date, duration, sid ---
...
def prepare_nc_files(
    nc_path: Path,
    storm_index,
):
    ds_track = read_single_storm_from_nc(nc_path, storm_index)
...
def read_nc_storm_metadata(
    nc_file: Path,
    storm_index: int,
) -> dict:
    """
    Read minimal metadata for a single storm from a NetCDF file
    without loading full arrays (slices only).

    Returns start/end dates based on valid (non-NaN) time steps.
    """
    ds_track = read_single_storm_from_nc(nc_file, storm_index)
    ...
    # Determine valid time steps
    core_vars = ["lon", "lat", "max_sustained_wind", "central_pressure", "environmental_pressure"]
    valid_mask = np.all([~np.isnan(ds_track[var].values) for var in core_vars], axis=0)
    time_dt_valid = time_dt[valid_mask]

    if len(time_dt_valid) == 0:
        raise ValueError(f"Storm {storm_index} has no valid time steps")
...
def check_next_storm_nc(
    ...
    if i + 1 < len(storm_indices):
        ...
        # Read metadata of the next storm from NC
        next_storm_metadata = read_nc_storm_metadata(nc_file, next_storm_index)
        ...
        if next_start_date < current_storm_end_nominal:
            ...
            log_storm_overlap(log_path, {
                ...
                "next_storm_end_date": next_storm_metadata["end_date"].isoformat(),
                "overlap_duration_days": overlap_duration_days,
            })

            # Load next storm
            next_tc_tracks = prepare_nc_files(
                nc_file,
...
def process_single_storm(storm_info):
    ...
    """
    (
        i, storm_index, nc_file, storm_indices, save_root,
        source_id, variant_label, experiment_id, batch_year, basin, draw
    ) = storm_info
    ...
    next_storm_speed, next_storm_index = check_next_storm_nc(
        i=i,
    ...
    days_impact = calculate_days_impact_single_storm_yearly(
        ...
        next_start_date=read_nc_storm_metadata(nc_file, next_storm_index)["start_date"] if next_storm_speed else None,
        max_impact_days=20,
    )
...
def main(
    ...
    storm_indices = get_ordered_storm_indices_nc(nc_file)

    storm_args = [
        (i, idx, nc_file, storm_indices, save_root,
        source_id, variant_label, experiment_id, batch_year, basin, draw)
