...
# Create a tool
tool = Tool(name="RR_CLIMAD1")


# Create a workflow, and set the executor
workflow = tool.create_workflow(
    name=f"RR_CLIMAD1_{wf_uuid}",
...
task_template = tool.get_task_template(
    template_name="RR_CLIMADA1_task",
    ...
    node_args=["source_id", "variant_label", "experiment_id", "batch_year", "basin", "draw", "relative_risk", "sample_name"],  # 👈 Include model_dict in node_args
                        ...
                        for relative_risk in RELATIVE_RISKS:
                            for sample_name in SAMPLE_NAME:
                                task = task_template.create_task(
                                    name=f"RR_CLIMADA1_{source_id}_{variant_label}_{experiment_id}_{batch_year}_{basin}_d{draw}_{relative_risk}_{sample_name}",
                                    source_id=source_id,
                                    variant_label=variant_label,
                                    experiment_id=experiment_id,
                                    batch_year=batch_year,
