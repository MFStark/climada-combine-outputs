...
parser.add_argument("--draw_batch", type=str, required=True, help="Draw batch number")
...
def save_raw_paf_raster_tif(
    ...
    # Convert to float32
    raw_paf = raw_paf.astype(np.float32)
    ...
    start_year, end_year = batch_year.split("-")
    filename = f"paf_{storm_draw}_{relative_risk}_{sample_name}_{basin}_{source_id}_{experiment_id}_{variant_label}_{start_year}01_{end_year}12_{year}{draw_text}.tif"
...
def save_raw_rr_raster_tif(
    ...
    # Convert to float32
    raw_rr = raw_rr.astype(np.float32)

    save_dir = save_root / storm_draw / source_id / variant_label / experiment_id / batch_year / str(year) / basin / "raw_rr"
    save_dir.mkdir(parents=True, exist_ok=True)

    draw_text = "" if tc_risk_draw == 0 else f"_e{tc_risk_draw - 1}"
    start_year, end_year = batch_year.split("-")
    filename = f"rr_{storm_draw}_{relative_risk}_{sample_name}_{basin}_{source_id}_{experiment_id}_{variant_label}_{start_year}01_{end_year}12_{year}{draw_text}.tif"
    save_path = save_dir / filename
    ...
    for year in all_years_in_draw(intensity_draw_store):
        ...
        for storm_ds in storms_in_year:
            ...
            try:
                ...
                print(f"⚠️ Storm {storm_id} has no days impact. Skipping...")
                continue  # skip to next storm
