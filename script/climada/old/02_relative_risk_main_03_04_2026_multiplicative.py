...
# SAVE_ROOT = Path("/mnt/share/scratch/users/mfiking/outputs/stage3")
...
def check_if_year_complete(
    ...
    """Return True if the yearly PAF parquet already exists."""
...
def process_single_year(args):
    ...
    # Reproject raster and shapefile safely
    ...
    for _, admin_shape in intersected_shapes.iterrows():
        ...
        # ----------------------------
        # Clip PAF raster to admin
        # ----------------------------
        paf_admin = raw_paf_raster.clip(admin_geom)
        paf_admin = paf_admin.mask(admin_geom, all_touched=True)
        ...
        pop_raster = pop_raster.mask(admin_geom, all_touched=True)
        pop_arr = pop_raster._ndarray.astype(np.float32, copy=False)

        # ----------------------------
        # Downsample PAF to pop grid
        # ----------------------------
        paf_resampled = paf_admin.resample_to(pop_raster)
        del paf_admin

        paf_resampled = paf_resampled.mask(admin_geom, all_touched=True)
        paf_arr = paf_resampled._ndarray.astype(np.float32, copy=False)
        ...
        # Valid cells
        # ----------------------------
        pop_mask = (pop_arr > 0) & np.isfinite(pop_arr) # positive, finite population
        paf_mask = (paf_arr > 0) & np.isfinite(paf_arr) # positive, finite PAF
        valid_mask = pop_mask & paf_mask # where pixel contains both valid population and paf values


        if pop_sum == 0 or not valid_mask.any():
            weighted_paf = 0.0
            population_exposed = 0.0
        else:
            weighted_paf = (pop_arr[valid_mask] * paf_arr[valid_mask]).sum() / pop_sum
            population_exposed = pop_arr[valid_mask].sum()
        ...
        # Cleanup immediately
        # ----------------------------
        del pop_raster, paf_resampled, pop_arr, paf_arr
        del pop_mask, paf_mask, valid_mask
        gc.collect()
...
def main(
    ...
    # apply permissions for all processed years
    for year in years:
        save_dir = (
            save_root
            / storm_draw
